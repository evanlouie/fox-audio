...
def calculate_gap(predictions, actuals, top_k=20):
    ...
    gap_calculator.accumulate(flatten(sparse_predictions), flatten(
        sparse_labels), sum(num_positives))
...
def top_k_by_class(predictions, labels, k=20):
    ...
    for video_index in range(predictions.shape[0]):
        prediction_triplets.extend(top_k_triplets(
            predictions[video_index], labels[video_index], k))
...
class EvaluationMetrics(object):
    ...
    def accumulate(self, predictions, labels, loss):
        ...
        self.map_calculator.accumulate(
