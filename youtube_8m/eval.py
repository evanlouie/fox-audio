...
if __name__ == "__main__":
    ...
    # Other flags.
    flags.DEFINE_integer("batch_size", 1024, "How many examples to process per batch.")
...
def build_graph(
    reader, model, eval_data_pattern, label_loss_fn, batch_size=1024, num_readers=1
):
...
def get_latest_checkpoint():
    index_files = file_io.get_matching_files(
        os.path.join(FLAGS.train_dir, "model.ckpt-*.index")
    )
    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                logging.info(
                    "examples_processed: %d | %s", examples_processed, iterinfo
                )
...
def evaluate():
    ...
    if not file_io.file_exists(model_flags_path):
        raise IOError(
            ("Cannot find file %s. Did you run train.py on the same " "--train_dir?")
            % model_flags_path
        )
    ...
    with tf.Graph().as_default():
        ...
        model = find_class_by_name(
            flags_dict["model"], [frame_level_models, video_level_models]
        )()
        label_loss_fn = find_class_by_name(flags_dict["label_loss"], [losses])()

        if FLAGS.eval_data_pattern is "":
            raise IOError(
                "'eval_data_pattern' was not specified. " + "Nothing to evaluate."
            )
