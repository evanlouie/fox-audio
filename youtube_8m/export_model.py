...
class ModelExporter(object):
    ...
    def build_inputs_and_outputs(self):
        if self.frame_features:
            ...
            def fn(x):
                return self.build_prediction_graph(x)
            ...
            video_id_output, top_indices_output, top_predictions_output = self.build_prediction_graph(
                serialized_examples
            )

        inputs = {
            "example_bytes": saved_model_utils.build_tensor_info(serialized_examples)
        }
    ...
    def build_prediction_graph(self, serialized_examples):
        video_id, model_input_raw, labels_batch, num_frames = self.reader.prepare_serialized_examples(
            serialized_examples
        )
