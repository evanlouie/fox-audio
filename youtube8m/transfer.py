# Authors:
# - Mihaela Curmei <Mihaela.Curmei@microsoft.com>
# - Yvonne Radsmikham <Yvonne.Radsmikham@microsoft.com>
# - Evan Louie <evan.louie@microsoft.com>
"""Binary for doing Transfer Learning on an LSTM YouTube-8M model."""
...
if __name__ == "__main__":
    ...
    # Other flags.
    flags.DEFINE_string("base_checkpoint", "model.ckpt-0", "pretrained checkpoint")
    flags.DEFINE_integer(
        "num_readers",
...
class TransferTrainer(Trainer):
    ...
    def run(self, start_new_model=False):
        ...
        with tf.Graph().as_default() as baseline_graph, tf.Session() as baseline_sess:
            ...
            baseline_graph.add_to_collections(
                "TRANSFERABLE_VARIABLES", baseline_transferable_variables
            )
        ...
        with tf.Graph().as_default() as graph:
            ...
            with tf.device(device_fn), tf.Session() as sess:
                if not meta_filename:
                    ...
                    transferable_variables = [
                        var
                        for var in graph.get_collection(
                    ...
                    for variable, weight in zip(
                        graph.get_collection("TRANSFERABLE_VARIABLES")[0],
                        baseline_transferable_weights[0],
                    ):
