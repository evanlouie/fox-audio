...
def padding(wav, white_noise_duration):
    ...
    for x in white_noise_duration:
        if x == 0:
            ...
            subprocess.call(['ffmpeg', '-i', wav, '-ar', "44100",
                             '-ac', "2", padded_fname+"_converted.wav", '-y'])
            ...
            wn.export(padded_fname+"_whitenoise.wav",
                      format="wav", parameters=["-ar", "16000"])

            # stitch white noise wav file to specific audio wav file
            # before
            new_wav_before = AudioSegment.from_wav(
                padded_fname+"_whitenoise.wav") + AudioSegment.from_wav(padded_fname+"_converted.wav")
            new_wav_before.export(padded_fname+"_padded"+"_"+str(white_noise_duration[1])+"_"+str(
                white_noise_duration[0])+".wav", format="wav", parameters=["-ar", "16000"])

            # after
            new_wav_after = AudioSegment.from_wav(
                padded_fname+"_converted.wav") + AudioSegment.from_wav(padded_fname+"_whitenoise.wav")
            new_wav_after.export(padded_fname+"_padded"+"_"+str(white_noise_duration[0])+"_"+str(
            ...
            wn_0.export(wav+"_whitenoise_0.wav", format="wav",
                        parameters=["-ar", "16000"])
            ...
            wn_1.export(wav+"_whitenoise_1.wav", format="wav",
                        parameters=["-ar", "16000"])
            ...
            new_wav_reverse = AudioSegment.from_wav(
                wav+"_whitenoise_1.wav") + AudioSegment.from_wav(wav) + AudioSegment.from_wav(wav+"_whitenoise_0.wav")
            new_wav_reverse.export(path+"/"+padded_fname+"_padded"+"_"+str(white_noise_duration[1])+"_"+str(
                white_noise_duration[0])+".wav", format="wav", parameters=["-ar", "16000"])
